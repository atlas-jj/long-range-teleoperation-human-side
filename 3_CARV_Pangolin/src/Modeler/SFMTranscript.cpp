...
namespace dlovi{
    namespace compvis{
        ...
        void SFMTranscript::processTranscriptText(){
            try{
                int nLineIndex = 0; //= parseTranscriptHeader();
        ...
        void SFMTranscript::stepTranscriptText(bool bFirstEntry){
            try{
                ...
                markAsValid(); // we're done.
                cout<<"nLineIndex "<<nLineIndex<<endl;
                //chris, clear lines.
                m_arrStrLine.clear();
                m_nNumEntries = 0;

                m_arrEntryText.clear();
                m_arrEntryType.clear();
            }
            catch(std::exception & ex){
                dlovi::Exception ex2(ex.what()); ex2.tag("SFMTranscript", "processTranscriptText"); ex2.raise();
            }
        ...
        void SFMTranscript::stepTranscriptCommandStr(const std::string & cmdStr){
          try{
              static int nLineIndex = 0;
              nLineIndex = stepTranscriptBody_cmd(cmdStr, m_enumStepEntryType, m_objStepEntryData, m_arrStepPoints, m_arrStepCamCenters, m_arrStepVisLists, nLineIndex);
              cout<<" line:"<<nLineIndex<<endl;
              // if(nLineIndex >= numLines())
              //     markAsValid(); // we're done.
          }
          catch(std::exception & ex){
              dlovi::Exception ex2(ex.what()); ex2.tag("SFMTranscript", "stepTranscriptCommandStr"); ex2.raise();
          }
        }
...
//          }
        ...
        int SFMTranscript::stepTranscriptBody_cmd(std::string strCurrentLine, EntryType & enumEntryType, EntryData & objEntryData, std::vector<dlovi::Matrix> & arrPoints,
                                              std::vector<dlovi::Matrix> & arrCamCenters, std::vector<std::vector<int> > & arrVisLists, int nLoop){
...
//                std::cout << strCurrentLine << endl;
                ...
                if(strCurrentLine == "}")
                    ...
                    prepareForNewEntry_Step(enumEntryType, ET_KEYFRAMEINSERTION);
                    std::vector<std::string> arrStr = split(strCurrentLine, "@");
                    cout<<"mark 1 , size:: "<<arrStr.size()<<endl;
                    Matrix matNewCam(trim(arrStr[0].substr(0, arrStr[0].length() - 1).substr(9))); // 9 == strlen("new cam: "), also remove the "{"
                    cout<<"mark 2"<<endl;
                    arrCamCenters.push_back(matNewCam); // new camera center
                    arrVisLists.push_back(std::vector<int>()); // empty visibility list, populated below
                    ...
                    {
                      strCurrentLine = trim(arrStr[i]);
                      cout<<strCurrentLine<<endl;
                      if(strCurrentLine.find("new point: ") != std::string::npos){
                          // eg: new point: [x; y; z], KF_ind1, KF_ind2, KF_ind3, ..., KF_indN
                          std::vector<std::string> arrStr = split(strCurrentLine.substr(11), ", "); // 11 == strlen("new point: ")
                    ...
                    {
                      int nCamIndex = (int)arrCamCenters.size() - 1; // the new camera
                      int nPointIndex = atoi(arrStr[1].c_str());//atoi(strCurrentLine.substr(13).c_str()); // 13 == strlen("observation: ")
                      m_objStepEntryData.arrPointIndices.push_back(nPointIndex);
                      arrVisLists[nCamIndex].push_back(nPointIndex);
                    }
                    // TODO: Remove me
                ...
                dlovi::Exception ex2(ex.what()); ex2.tag("SFMTranscript", "stepTranscriptBody_cmd"); ex2.raise();
            }
        }
